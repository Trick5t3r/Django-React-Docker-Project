...
from .models import BaseFile, PDFFile, ExcelFile, ImageFile, AudioFile, VideoFile, OtherFile
...
class ExcelFileSerializer(BaseFileSerializer):
    ...
    def create(self, validated_data):
        # 'sessionid' has already been handled in validate
        validated_data.pop('sessionid', None)
        return ExcelFile.objects.create(**validated_data)
...
class AudioFileSerializer(BaseFileSerializer):
    class Meta(BaseFileSerializer.Meta):  # Hériter des champs et des options Meta
        model = AudioFile
